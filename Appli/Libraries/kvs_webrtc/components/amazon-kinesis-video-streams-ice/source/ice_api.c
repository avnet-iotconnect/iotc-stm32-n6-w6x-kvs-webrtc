                                    ...
                                    const IceRemoteCandidateInfo_t * pRemoteCandidateInfo )
{
    ...
    if( ( pContext == NULL ) ||
        ...
        ( pRemoteCandidateInfo->pEndpoint == NULL ) )
    {
        result = ICE_RESULT_BAD_PARAM;
    }
                                          ...
                                          size_t * pMessageBufferLength )
{
    ...
    if( ( pContext == NULL ) ||
        ( pIceCandidatePair == NULL ) ||
        ( pIceCandidatePair->pLocalCandidate == NULL ) ||
                                              ...
                                              size_t * pTotalBufferLength )
{
    ...
    if( result == ICE_RESULT_OK )
    {
        ICE_WRITE_UINT16( &( pTurnBuffer[ ICE_TURN_CHANNEL_DATA_MESSAGE_CHANNEL_NUMBER_OFFSET ] ),
                          pIceCandidatePair->turnChannelNumber );
        ICE_WRITE_UINT16( &( pTurnBuffer[ ICE_TURN_CHANNEL_DATA_MESSAGE_LENGTH_OFFSET ] ),
                          turnPayloadLength );

        if( padding > 0 )
        {
            memset( &( pTurnBuffer[ ICE_TURN_CHANNEL_DATA_MESSAGE_HEADER_LENGTH + turnPayloadLength ] ), 0, padding );
        }
