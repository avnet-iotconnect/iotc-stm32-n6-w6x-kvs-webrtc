...
/*
 * When lwIP is built without IPv6 (LWIP_IPV6 == 0) the socket layer does not
 * define struct sockaddr_in6 or INET6_ADDRSTRLEN.  The ice_controller sources
 * reference these types even in code-paths that are not taken for IPv4-only
 * builds, so provide minimal stubs here.
 *
 * Note: struct in6_addr is always defined in lwip/inet.h (unconditionally),
 * so we must NOT redefine it — only sockaddr_in6 and INET6_ADDRSTRLEN are
 * missing when LWIP_IPV6 == 0.
 */
#include "lwip/opt.h"
#include "lwip/inet.h"   /* struct in6_addr is always here */
#if !LWIP_IPV6
#  ifndef INET6_ADDRSTRLEN
#    define INET6_ADDRSTRLEN 46
#  endif
#  ifndef ICE_CONTROLLER_SOCKADDR_IN6_STUB_DEFINED
...
#  endif /* ICE_CONTROLLER_SOCKADDR_IN6_STUB_DEFINED */
#endif /* !LWIP_IPV6 */
