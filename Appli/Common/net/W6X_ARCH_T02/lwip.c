...
/* sys_now() is already provided by the project's LwIP port (sys_arch.c) */
...
static void netif_status_callback(struct netif *netif_cur)
{
  ...
  if (netif_cur->ip_addr.addr != xLastAddr.addr
#if (LWIP_IPV6_DHCP6 == 1)
      || (netif_cur->ip_addr.u_addr.ip6.addr != xLastAddr.u_addr.ip6.addr)
#endif
     )
  {
    APP_setevent(&app_evt_current, EVT_APP_WIFI_GOT_IP);
