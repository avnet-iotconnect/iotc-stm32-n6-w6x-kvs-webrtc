...
/* Sensor-side start only (ISP_Init + ISP_Start + IMX335 MODE_SELECT=streaming).
 * For callers that drive the DCMIPP pipe through HAL directly (e.g. semi-planar
 * DBM) and just need the middleware to initialise its ISP handle and kick the
 * sensor into streaming.  Idempotent via the is_camera_started guard. */
